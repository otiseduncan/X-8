...
def test_model_owned_lanes_do_not_auto_capture_x8_memory() -> None:
    kernel = object.__new__(XV8Kernel)
    assert kernel._x8_memory_capture_allowed("conversation_repair") is False
    assert kernel._x8_memory_capture_allowed("normal_chat") is False
    ...
    assert kernel._x8_memory_capture_allowed("github_status") is True
