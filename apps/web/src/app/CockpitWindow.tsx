...
export function CockpitWindow() {
  ...
  async function refreshFiles(nextPath?: string) {
    try { const payload = await loadFiles(); const nextFiles = payload.data ?? []; setFiles(nextFiles); const fallback = nextFiles.find((file) => file.kind === 'file')?.path ?? ''; const pathToOpen = nextPath || selectedPath || fallback; setStatus(`workspace ready · ${nextFiles.length} visible entries`); if (pathToOpen) await openFile(pathToOpen); } catch (error) { setStatus(error instanceof Error ? error.message : 'workspace unavailable'); }
  }

  async function openFile(path: string) {
    setSelectedPath(path); setProposal(null);
    try { const payload = await readFile(path); setDraft(payload.data.content); setStatus(`opened ${path}`); if (canPreview(path)) setPreviewStamp(Date.now()); } catch (error) { setDraft(''); setStatus(error instanceof Error ? error.message : `could not open ${path}`); }
  }
  ...
  async function applyDraft() {
    if (!selectedPath) return;
    try { const payload = await applyUpdate(selectedPath, draft, true); setProposal(payload.data); setStatus(payload.message || 'approved edit applied inside workspace root'); setPreviewStamp(Date.now()); await refreshFiles(selectedPath); } catch (error) { setStatus(error instanceof Error ? error.message : 'apply failed'); }
  }
