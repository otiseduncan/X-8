...
import { CHAT_TIMEOUT_MS, createArtifactPreview, proposeUpdate, readFile, requestImage, runSearch, runSelfBuildPrompt, sendChat } from '../../services/apiClient';
...
export interface ChatConversationHandlerDeps {
  entry: string;
  attachments: AttachmentReference[];
  sessionId: string | undefined;
  muted: boolean;
  userInteracted: MutableRefObject<boolean>;
  ...
  nowId: () => string;
  recordResponseLifecycle: (kind: string, hasText: boolean, hasCards: boolean) => void;
...
export function createChatConversationHandlers(deps: ChatConversationHandlerDeps) {
  const { entry, attachments, sessionId, muted, userInteracted, requestStartedAt, stickToLatestRef, appendMessage, createGitHubCards, finishAssistantResponseLifecycle, nowId, recordResponseLifecycle, setAttachments, setChatPending, setCode, setEntry, setError, setLastApiError, setLastApiStatus, setLastTimeoutReason, setLatestReceipt, setLatestResult, setProposal, setSelectedPath, setSessionId, setStage, setUserAwayFromLatest } = deps;
  ...
  async function createAssistantReply(text: string, outgoingAttachments: AttachmentReference[] = []) {
    try {
      ...
      appendMessage({ id: response.data.message_id || nowId(), role: 'assistant', text: responseText, attachments: response.data.attachments, cards: responseCards });
  ...
  async function openFileCard(path: string) {
    try {
      ...
      appendMessage({ id: nowId(), role: 'assistant', text: `Opened ${path}.`, cards: [{ id: nowId(), type: 'file', title: path, status: response.status, summary: `${response.data.line_count} lines loaded into an inline file viewer.`, receipt: response.receipts?.[0], payload: { path, content: response.data.content } }] });
  ...
  async function proposeDiffCard(path: string) {
    try {
      ...
      appendMessage({
        ...
        cards: [
          { id: nowId(), type: 'editor', title: `Editor draft: ${path}`, status: 'draft', summary: 'The draft is local to this chat card. Applying it requires click approval.', payload: { path, content: next }, collapsed: true },
          { id: nowId(), type: 'diff', title: `Diff proposal: ${path}`, status: response.data.receipt.status, summary: response.data.receipt.summary, receipt: response.data.receipt, payload: { path, diff: response.data.diff, risk: response.data.approval?.risk || 'medium', beforeAfter: response.data.approval?.intent.before_after_summary || 'Content would change only after approval.', approvalState: response.data.approval?.status || 'approval_required' } },
          { id: nowId(), type: 'approval', title: 'Apply requires approval', status: response.data.approval?.status || 'pending', summary: 'Click Apply inside this card to open the focused approval dialog.', receipt: response.data.receipt, payload: { path } }
        ]
      });
      setStage(muted ? 'muted' : 'idle');
    } catch {
  ...
  async function createArtifactCard(prompt: string) {
    try {
      ...
      appendMessage({ id: nowId(), role: 'assistant', text: 'Generated an inline website artifact preview.', cards: [{ id: nowId(), type: 'artifact', title: 'Inline website preview', status: response.status, summary: 'Preview, code, and metadata are attached to this chat response.', receipt: response.receipts?.[0], payload: { html: response.data.html, css: response.data.css, metadata: { title: response.data.title, exportable: true } } }] });
      setStage(muted ? 'muted' : 'idle');
    } catch {
      setStage('error');
  ...
  async function createSelfBuildCards(prompt: string) {
    try {
      ...
      if (!detail.patch_hash) {
        appendMessage({ id: nowId(), role: 'assistant', text: response.message, cards: [{ id: nowId(), type: 'receipt', title: 'Self-build status', status: response.status, summary: response.message, payload: response.data, collapsed: false }] });
        setStage(muted ? 'muted' : 'idle');
        return;
      }
  ...
  async function createResearchCard(query: string) {
    try {
      ...
      appendMessage({ id: nowId(), role: 'assistant', text: results.length ? 'Search results are attached inline.' : 'Search was attempted; provider status is shown inline.', cards: [{ id: nowId(), type: 'research', title: 'Inline research results', status: response.status, summary: response.message, receipt: response.receipts?.[0], payload: { query, provider: response.data.provider || 'SearXNG', results, freshness: 'Runtime provider response; verify dates in cited sources for time-sensitive claims.' } }] });
      setStage(muted ? 'muted' : 'idle');
    } catch {
      setStage('error');
      appendMessage({ id: nowId(), role: 'assistant', text: 'Search is unavailable right now.', cards: [errorCard('Provider unavailable', 'SearXNG search could not be reached. No search results were invented.')] });
    }
  }
  ...
  async function createImageCard(prompt: string) {
    try {
      ...
      appendMessage({ id: nowId(), role: 'assistant', text: 'Image generation status is attached inline.', cards: [{ id: nowId(), type: response.status === 'ok' ? 'image' : 'error', title: response.status === 'ok' ? 'Inline image result' : 'Image generation unavailable', status: response.status, summary: response.status === 'ok' ? response.message : 'Reason: ComfyUI service unreachable or Juggernaut model missing. No image was generated.', receipt: response.receipts?.[0], payload: { prompt, model: 'Juggernaut', workflow: 'ComfyUI default', seed: response.data.seed || 'pending', imageUrl: response.data.image_url || '' } }] });
      setStage(muted ? 'muted' : 'idle');
    } catch {
      setStage('error');
      appendMessage({ id: nowId(), role: 'assistant', text: 'Image generation unavailable.', cards: [errorCard('Image generation unavailable', 'Reason: ComfyUI service unreachable or Juggernaut model missing. No image was generated.')] });
    }
  }

  function createTestCard(text: string) {
    setLatestResult('Test run queued for approval');
    appendMessage({ id: nowId(), role: 'assistant', text: 'Test execution needs approval before Docker work starts.', cards: [{ id: nowId(), type: 'test', title: 'Docker test run', status: 'approval_required', summary: `Requested: ${text}. No test command has run yet.`, receipt: { id: nowId(), action: 'test_run', status: 'approval_required', summary: 'Click approval is required before execution.' }, payload: { command: 'docker compose run --rm api-tests' } }] });
    setStage(muted ? 'muted' : 'idle');
  }

  return { submitMessage, proposeDiffCard };
}
