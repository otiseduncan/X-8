...
test('renders assistant mode without permanent dashboard panels', async () => {
  ...
  expect(screen.queryByRole('button', { name: /^Mute$/i })).not.toBeInTheDocument(); expect(screen.queryByRole('button', { name: /^Read aloud$/i })).not.toBeInTheDocument(); expect(screen.getAllByRole('button', { name: /^Info/i })).toHaveLength(1);
  ...
  expect(screen.getByText('postgres')).toBeInTheDocument(); expect(screen.getAllByText('auto_saved').length).toBeGreaterThan(0);
});
test('Brain memory panel searches filters opens detail and runs actions', async () => {
  ...
  expect(screen.getByLabelText('Continuity panel')).toBeInTheDocument(); expect((await screen.findAllByText('Answer preference')).length).toBeGreaterThan(0);
...
test('renders generated artifacts and file viewers inline', async () => {
  render(<App />);
  await send('make a simple HTML website preview');
  const artifactCard = await screen.findByTestId('inline-artifact-card');
  expect(within(artifactCard).getByTestId('artifact-package-header')).toBeInTheDocument();
  const approve = within(artifactCard).getByRole('button', { name: /^Approve$/ });
  const apply = within(artifactCard).getByRole('button', { name: /^Apply$/ });
...
test('supports package top tabs and page tabs for multi-page artifacts', async () => {
  ...
  const artifactCard = await screen.findByTestId('inline-artifact-card');
  expect(within(artifactCard).queryByRole('button', { name: 'Pages' })).not.toBeInTheDocument();
  ['Preview', 'Code', 'Files', 'Assets', 'Metadata', 'History/Log'].forEach((name) => {
    expect(within(artifactCard).getByRole('button', { name })).toBeInTheDocument();
  });
  ['index', 'about', 'services', 'contact'].forEach((name) => {
    expect(within(artifactCard).getByRole('button', { name })).toBeInTheDocument();
  });
  ...
  expect(within(artifactCard).getByRole('button', { name: /^Approve$/ })).toBeInTheDocument();
  expect(within(artifactCard).getByRole('button', { name: /^Apply$/ })).toBeInTheDocument();
});
